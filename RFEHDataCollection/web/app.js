...
const els = {
  ...
  recordScenario: document.getElementById("recordScenario"),
  recordDevice: document.getElementById("recordDevice"),
  recordRouter: document.getElementById("recordRouter"),
  recordApp: document.getElementById("recordApp"),
...
function updateRecording() {
  ...
  const context = recording.experimentContext || {};
  ...
  els.recordScenario.textContent = context.environmentId || context.environment_id || "--";
  els.recordDevice.textContent = context.deviceId || context.device_id || "--";
  els.recordRouter.textContent = context.routerId || context.router_id || "--";
  els.recordApp.textContent = context.appLabel || context.app_label || context.appId || context.app_id || "--";
  els.recordOutput.textContent = recording.outputDir || "--";

  if (recording.error) {
    setRecordMessage(recording.error, true);
  } else if (saving) {
    setRecordMessage("Saving raw CSV and voltage plot...");
  } else if (active) {
    setRecordMessage(`Writing ${recording.csvPath || "raw CSV"}...`);
  } else if (completed && recording.result) {
    const outputs = [recording.result.csvPath, recording.result.pngPath].filter(Boolean);
