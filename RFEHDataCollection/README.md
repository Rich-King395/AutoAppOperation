...
7. The live web monitor can record experiments through its HTTP API and save raw CSV, PNG plot, and metadata.
...
- Records experiments through the HTTP API without opening a second serial connection.
- Shows automated recording status in the browser for AppCollector-driven experiments.
...
- `app.js`: WebSocket client, live time-domain Canvas chart, and automated recording status.
- Recording status: state, elapsed time, sample count, experiment context, and output paths.
...
`Time(ms)` is the PC-side elapsed recording time, measured from the start of the recording.
